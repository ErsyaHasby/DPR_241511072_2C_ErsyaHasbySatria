...
class PenggajianModel extends Model
{
    ...
    {
        // Query dasarnya sama seperti getPenggajian()
        $builder = $this->db->table('anggota a');
        $builder->select("
        a.id_anggota, a.gelar_depan, a.nama_depan, a.nama_belakang, a.gelar_belakang,
        a.jabatan, a.status_pernikahan, a.jumlah_anak
    ");
        ...
        $subQuery = $this->db->table('penggajian p')
            ->join('komponen_gaji k', 'p.id_komponen_gaji = k.id_komponen_gaji')
            ->select("SUM(CASE WHEN k.nama_komponen = 'Tunjangan Istri/Suami' AND a.status_pernikahan != 'Kawin' THEN 0 WHEN k.nama_komponen = 'Tunjangan Anak' AND a.jumlah_anak = 0 THEN 0 WHEN k.nama_komponen = 'Tunjangan Anak' AND a.jumlah_anak > 0 THEN k.nominal * LEAST(a.jumlah_anak, 2) ELSE k.nominal END)")
            ->where('p.id_anggota = a.id_anggota')
            ->where('k.satuan', 'Bulan')
            ->getCompiledSelect();
        ...
        // Kondisi pencarian yang disederhanakan
        if ($keyword) {
            $builder->groupStart();
            $builder->like('a.nama_depan', $keyword);
            $builder->orLike('a.nama_belakang', $keyword);
            $builder->groupEnd();
        }
